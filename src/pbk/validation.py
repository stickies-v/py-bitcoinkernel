...
default_validation_callbacks = ValidationInterfaceCallbacks(
    user_data=None,
    block_checked = lambda user_data, block, state : print(f"block_checked: block: {block}, state: {state}")
)

class ValidationInterface(KernelOpaquePtr):
    def __init__(self, callbacks: ValidationInterfaceCallbacks = default_validation_callbacks):
